...
func TestHelperFunctions(t *testing.T) {
	...
	rootFolders := []*starrradarr.RootFolder{{ID: 1, Path: "/movies-a"}, {ID: 2, Path: "/movies-b"}}
	...
	if folder, ok := matchRootFolder(rootFolders, "/movies-a"); !ok || folder.ID != 1 {
...
func TestFindMovie(t *testing.T) {
	...
	t.Run("tmdb hit", func(t *testing.T) {
	...
	t.Run("tmdb miss then title hit", func(t *testing.T) {
		client := &Client{api: fakeRadarrAPI{getMovieContext: func(_ context.Context, query *starrradarr.GetMovie) ([]*starrradarr.Movie, error) {
			...
			return []*starrradarr.Movie{{Title: "Movie Title"}}, nil
		}}}
		match, err := client.FindMovie(ctx, "Movie Title", 123)
		if err != nil {
			t.Fatalf("FindMovie() error = %v", err)
		}
	...
	t.Run("blank title with no tmdb match returns not found", func(t *testing.T) {
	...
	t.Run("sdk errors wrapped", func(t *testing.T) {
...
func TestResolveAddMovieDefaults(t *testing.T) {
	...
	client := &Client{api: fakeRadarrAPI{
		getRootFoldersContext: func(context.Context) ([]*starrradarr.RootFolder, error) {
			return []*starrradarr.RootFolder{{ID: 1, Path: "/movies", Accessible: true}}, nil
		},
	...
	if defaults.RootFolderPath != "/movies" || defaults.QualityProfileID != 2 || defaults.QualityProfileName != "HD" {
	...
	client = &Client{api: fakeRadarrAPI{getRootFoldersContext: func(context.Context) ([]*starrradarr.RootFolder, error) {
		return []*starrradarr.RootFolder{{ID: 1, Path: "/movies-a", Accessible: true}, {ID: 2, Path: "/movies-b", Accessible: true}}, nil
	}}}
	if _, err := client.resolveRootFolder(ctx, ""); err == nil {
		t.Fatal("resolveRootFolder() error = nil, want error")
	}
	client = &Client{api: fakeRadarrAPI{getRootFoldersContext: func(context.Context) ([]*starrradarr.RootFolder, error) {
		return []*starrradarr.RootFolder{{ID: 1, Path: "/movies-a", Accessible: false}}, nil
	}}}
	if _, err := client.resolveRootFolder(ctx, ""); err == nil {
	...
	if folder, err := client.resolveRootFolder(ctx, "/movies-a"); err != nil || folder.Path != "/movies-a" {
...
func TestCreateAndPreviewMovie(t *testing.T) {
	ctx := context.Background()
	defaults := AddMovieDefaults{RootFolderPath: "/movies", QualityProfileID: 7, QualityProfileName: "HD"}
	...
	t.Run("missing tmdb id", func(t *testing.T) {
		client := &Client{api: fakeRadarrAPI{}}
	...
	t.Run("lookup by tmdb", func(t *testing.T) {
		...
		if movie.Title != "Movie" || got == nil || got.RootFolderPath != "/movies" || got.QualityProfileID != 7 || !got.AddOptions.SearchForMovie {
	...
	t.Run("missing defaults validation", func(t *testing.T) {
...
func TestUpdateAndPreviewMovieMonitoring(t *testing.T) {
	ctx := context.Background()

	t.Run("nil movie", func(t *testing.T) {
		client := &Client{api: fakeRadarrAPI{}}
	...
	t.Run("unchanged returns existing movie", func(t *testing.T) {
	...
	t.Run("fetches stored movie and updates", func(t *testing.T) {
		called := false
		client := &Client{api: fakeRadarrAPI{
